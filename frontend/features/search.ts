...
import { ge, hide, show, toast } from "../core/utils";
...
export function updateChips(): void {
	...
	for (const [id, lbl] of checks) {
		if ((ge(id) as HTMLInputElement).checked)
			chips.push(
				`<span class="chip">${lbl}<button type="button" class="chip-x" onclick="document.getElementById('${id}').checked=false;window.__updateChips()"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg></button></span>`,
			);
	}
	...
	for (const [id, lbl] of nums) {
		...
		if (v)
			chips.push(
				`<span class="chip">${lbl}: ${v}<button type="button" class="chip-x" onclick="document.getElementById('${id}').value='';window.__updateChips()"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg></button></span>`,
			);
	}
	...
	if (cat)
		chips.push(
			`<span class="chip">Category: ${cat}<button type="button" class="chip-x" onclick="document.getElementById('category').value='';window.__updateChips()"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg></button></span>`,
		);
