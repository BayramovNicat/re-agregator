...
export function render(): void {
	...
	let newCount = 0;
	for (const p of list) {
		const bm = state.bookmarks.has(p.source_url);
		const el =
			state.currentView === "grid" ? buildCard(p, bm, cb) : buildRow(p, bm, cb);
		if (state.renderedSet.has(p.source_url)) {
			el.style.animation = "none";
		} else {
			el.style.animationDelay = `${Math.min(newCount, 15) * 22}ms`;
			state.renderedSet.add(p.source_url);
			newCount++;
		}
		wrap.appendChild(el);
	}
